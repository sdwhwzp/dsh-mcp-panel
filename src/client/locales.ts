...
export const zh = {
  ...
  removeServer: '删除（停用）',
  enableServer: '重新启用',
  removeConfirm: '删除 = 追加一条 `disabled: true` 覆盖操作（patch 词汇表没有 remove，本控制台也从不改写既有内容）。条目会一直留在 patch 文件里，这里的「重新启用」可随时恢复；要真正去掉它，只能手工编辑 cordis.patch.yml。',
...
export const en = {
  ...
  removeConfirm: 'Removal = appending one `disabled: true` override operation (the patch vocabulary has no remove, and this console never rewrites existing content). The row stays in the patch file for good; Re-enable brings it back, and dropping it for real means hand-editing cordis.patch.yml.',
