...
export const zh = {
  ...
  statusDisposed: '已卸载',
  statusRegistered: '工具已注册',
  statusNoTools: '已加载·无工具',
  reconnects: '重连',
  lastError: '最近错误',
  none: '—',
  fiber: 'Cordis fiber',
  ...
  filterTools: '过滤工具',
  ms: 'ms',
  derivedNote: '连接状态/重连计数来自上游 mcp/status 事件；未观察到上游数据时，状态按 Cordis fiber 与工具注册表推导：fiber 活跃且已注册工具为"工具已注册"，活跃但无工具为"已加载·无工具"，fiber 未活跃为"未知"。',
  probes: '连通性探测',
  ...
  enableServer: '重新启用',
  deleteServer: '删除',
  deleteConfirm: '删除会把该服务器的所有 patch 操作从 cordis.patch.yml 中移除（只删整行，其余内容逐字节保留，写入前自动备份）。此操作不可撤销，只能从备份恢复。',
  removeConfirm: '停用 = 追加一条 `disabled: true` 覆盖操作，条目留在文件里，随时可以「重新启用」。要把它从文件里去掉，用「删除」。',
...
export const en = {
  ...
  statusRegistered: 'Tools registered',
  statusNoTools: 'Loaded, no tools',
  ...
  ms: 'ms',
  derivedNote: 'Connection status and reconnect counts come from the upstream mcp/status seam; without observed upstream data the state is derived from the Cordis fiber and the tool registry: an active fiber with registered tools reads "Tools registered", active without tools reads "Loaded, no tools", and a fiber that is not active reads "Unknown".',
  ...
  editServer: 'Edit',
  removeServer: 'Disable',
  enableServer: 'Re-enable',
  deleteServer: 'Delete',
  deleteConfirm: 'Delete removes every patch operation for this server from cordis.patch.yml (whole lines only; everything else stays byte-for-byte, and a timestamped backup is taken first). It cannot be undone except from that backup.',
  removeConfirm: 'Disable appends one `disabled: true` override operation; the row stays in the file and Re-enable brings it back. To take it out of the file, use Delete.',
